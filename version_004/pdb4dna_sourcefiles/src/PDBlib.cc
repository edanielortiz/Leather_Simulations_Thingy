  ...
  else
  {
    ...
    while(!infile.eof())
    {
      ...
      else if((sLine.substr(0, 6)).compare("ATOM  ") == 0)
      {
        ...
        /************ Begin ATOM ******************/
        //serial
        istringstream((sLine.substr(8, 20))) >> serial;

        //To be improved
        //atomName :
        //atomName = sLine.substr(12, 4);
        element = sLine.substr(sLine.length()-4, 1);
        helper = sLine;
        ...
        {
#ifndef GEANT4
          G4cerr << "Element not recognized : " << element << helper << G4endl;
          G4cerr << "Stop now" << G4endl;
          exit(1);
#else
          G4ExceptionDescription errMsg;
          errMsg << "Element not recognized : " << element << helper << G4endl;
...

        {
          ...
          //x,y,z :
          stringstream((sLine.substr(52, 20))) >> x;
          stringstream((sLine.substr(78, 20))) >> y;
          stringstream((sLine.substr(104, 20))) >> z;
          //occupancy :
          occupancy = 1.;
