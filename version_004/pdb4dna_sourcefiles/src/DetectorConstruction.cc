  ...
  fCheckOverlaps(false)
{
  ...
  //
  fPdbFileName=G4String("geometry.pdb");
  fPdbFileStatus=0;
  fChosenOption=11;
...
{
  // Geometry parameters
  //G4double worldSize  = 50000*1*angstrom;
  ...
  // World
  //
G4double world_x_size = 5000.0*1*angstrom;
G4double world_y_size = 5000.0*1*angstrom;
G4double world_z_size = 5000.0*1*angstrom;

  G4VSolid* worldS
  = new G4Box("World",           // its name
              world_x_size/2, world_y_size/2, world_z_size/2); // its size
  ...
  while (moleculeListTemp)
  {
    ...
    while (residueListTemp)
    {
      ...
      for (int i=0 ; i < (upTo + startFrom) ; i++) //Phosphat or Sugar or Base
      {
        ...
        {
          nbAtomN++;
          //G4cout << AtomTemp->fX << "\t" << AtomTemp->fY << "\t" << AtomTemp->fZ << G4endl;
          new G4PVPlacement(0,
                            G4ThreeVector(AtomTemp->fX*1*angstrom,
                                          AtomTemp->fY*1*angstrom,
                                          AtomTemp->fZ*1*angstrom),
